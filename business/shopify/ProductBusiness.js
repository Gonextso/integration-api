...
export default class ShopifyProductBusiness extends CoreClass {
    ...
    /**
     * Sync a single product (for queue processing)
     */
    syncSingleProduct = async (product, categoryList = []) => {
        try {
            const mutation = productMutations.sync;
            const productSizeOp = new Set();
            const productColorOp = new Set();
            ...
            // Limit check for non-ENTERPRISE plans
            if (this.tenant.shopify.billing.planKey !== SystemCodes.BILLING_PLANS.ENTERPRISE.KEY) {
                ...
                // ENTERPRISE plan - no limit check, just collect variants
                for (const variant of product.variants) {
                    productColorOp.add(variant.color);
                    productSizeOp.add(variant.dimention);
                }
            ...
            if (!data || data.errors || data.userErrors || (data.data.productSet.userErrors && data.data.productSet.userErrors.length > 0)) {
                const errorMsg = JSON.stringify(data.errors || data.data.productSet.userErrors);
            ...
            if (this.tenant.shopify.isInventoryTracking) {
                await this.#setProductVariantsToTracked(shopifyProduct);
            }

            // Update SyncedBarcode records and track new variants for limit usage
            ...
            for (let index = 0; index < product.variants.length; index++) {
                ...
                // Check if this is a new variant (wasn't synced before)
                const wasNewVariant = !existingSync;
                ...
                await SyncedBarcode.updateOne(
                    ...
                    },
                    {
                        barcode: variant.barcode,
                        tenant: this.tenant.id,
                        productId: shopifyProduct.id ?? existingProductId ?? existingSync?.productId ?? null,
                        variantId: variantNode.id ?? existingSync?.variantId ?? null
                    },
                    { upsert: true }
                );

                // Track new variants for limit usage
                if (wasNewVariant) {
                    newVariantCount++;
                }
            }

            // Update limit usage for all plans (only for new variants)
            // ENTERPRISE plan has no limit blocking but still tracks usage
            if (newVariantCount > 0) {
                const limitBusiness = new LimitBusiness(await Tenant.findById(this.tenant.id));
                await limitBusiness.useLimit(SystemCodes.LIMIT_TYPE.PRODUCT_DETAILS, newVariantCount);
            }
            ...
            this.logger.error(new Error(`Error syncing single product ${product.erp_id}: ${error.message}`));
            return {
                ok: false,
